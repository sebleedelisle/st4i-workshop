...
void setup() {
	...
	client.setDataArrivedDelegate(ondata);
        Serial.println("attempt to connect to websocket"); 
	while (!client.connect(cc3000, hostname, port)) Serial.println(F("Not connected."));
        Serial.println("CONNECTED TO WEBSOCKET!"); 
      	if (client.connected()) client.sendMessage("chat message");
}
...
void loop() {
	...
	if ((now - lasthello) >= HELLO_INTERVAL) {
		lasthello = now;
		if (client.connected()) client.sendMessage("Hello, world!\n");
	}
}
