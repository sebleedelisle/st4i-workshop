...
void setup() { 
  ...
  smoothedNumber = analogRead(knobPin); 
...
void loop() {
  ...
  int hue = map(analogRead(knobPin),0,1024,0,360);
  ...
  smoothedNumber += (double) (analogRead(knobPin)-smoothedNumber) * 0.1f; 
  showNumber(smoothedNumber);
  delay(10);
